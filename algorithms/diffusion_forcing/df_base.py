...
class DiffusionForcingBase(BasePytorchAlgo):
    def __init__(self, cfg: DictConfig):
        ...
        self.anchor_prefix_size = cfg.anchor_prefix_size
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        if self.cfg.noise_level == "context_amortization" and self.anchor_prefix_size > 0:
            # zero loss on the clean anchor prefix; masks shape is (n_frames_raw, B)
            k_raw = self.anchor_prefix_size * self.frame_stack
            masks = masks.clone()
            masks[:k_raw] = 0.0
    ...
    def _generate_noise_levels(self, xs: torch.Tensor, masks: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        match self.cfg.noise_level:
            ...
            case "context_amortization":
                # clean anchor prefix + random-noise suffix; matches the noise pattern seen at inference
                noise_levels = torch.randint(0, self.timesteps, (num_frames, batch_size), device=xs.device)
                if self.anchor_prefix_size > 0:
                    # inference scales "clean" context to stabilization_level - 1 via q_sample; match that here
                    noise_levels[: self.anchor_prefix_size] = self.stabilization_level - 1
