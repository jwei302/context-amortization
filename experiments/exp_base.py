...
class _LossCSVCallback(Callback):
    """Minimal CSV dump of training/loss per step. Survives offline wandb breakage."""
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if isinstance(outputs, dict) and "loss" in outputs:
            ...
            loss = outputs
        if loss is not None:
            self._fh.write(f"{trainer.global_step},{float(loss.detach().item()):.6f}\n")
...
class BaseLightningExperiment(BaseExperiment):
    ...
    def training(self) -> None:
        ...
        callbacks.append(_LossCSVCallback(output_dir / "loss.csv"))
